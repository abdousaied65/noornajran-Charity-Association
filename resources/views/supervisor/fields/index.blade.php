        ...
        <div class="col-xl-12">
            <div class="card">
                ...
                <div class="row mt-1 mb-1 text-center justify-content-center align-content-center">
                    <form method="GET" action="{{route('print.selected.fields')}}">
                        <button type="submit" class="btn btn-md btn-warning m-1 print_selected">
                            <i class="fa fa-print"></i>
                            طباعة
                        </button>
                    </form>
                    <form method="POST" action="{{route('export.fields.excel')}}">
                        @csrf
                        @method('POST')
                    ...
                    <form method="POST" class="" id="myForm" action="{{route('remove.selected.fields')}}">
                        @csrf
                        @method('POST')
                    ...
                    <a href="{{route('supervisor.fields.create')}}" role="button"
                    ...
                    <div class="table-responsive hoverable-table">
                        <table class="display text-nowrap w-100 table-bordered" id="example-table"
                            ...
                            @foreach ($data as $field)
                                <tr>
                                    <td>
                                        <input class="check" name="fields[]" form="myForm"
                                               value="{{$field->id}}"
                                               type="checkbox">
                                    </td>
                                    <td>{{ ++$i }}</td>
                                    ...
                                    <td>
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu">
                                                @can('عرض مجال')
                                                    <a href="{{ route('supervisor.fields.show', $field->id) }}"
                                                       class="dropdown-item">
                                                        <i class="fa fa-eye"></i>
                                                        عرض
                                                    </a>
                                                @endcan
                                                    @can('تعديل مجال')
                                                        <a href="{{ route('supervisor.fields.edit', $field->id) }}"
                                                           class="dropdown-item">
                                                    ...
                                                    @endcan
                                                @can('حذف مجال')
                                                    <a class="dropdown-item delete_field"
                                                       field_id="{{ $field->id }}"
                                                       field="{{ $field->field }}" data-toggle="modal"
                                                       href="#modaldemo8">
                                                        <i class="fa fa-trash"></i>
                                                        حذف
                                                    </a>
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content modal-content-demo">
                    <div class="modal-header text-center">
                        <h6 class="modal-title w-100" style="font-family: 'Cairo'; ">حذف مجال</h6>
                        <button aria-label="Close" class="close"
                                data-dismiss="modal" type="button"><span aria-hidden="true">&times;</span></button>
                    </div>
                    <form action="{{ route('supervisor.fields.destroy', 'test') }}" method="post">
                        {{ method_field('delete') }}
                        {{ csrf_field() }}
                        <div class="modal-body">
                            <p>هل انت متأكد انك تريد الحذف ؟</p><br>
                            <input type="hidden" name="field_id" id="field_id" value="">
                            <input class="form-control" name="field" id="field" type="text" readonly>
                        </div>
...
<script>
    $(document).ready(function () {
        ...
        $('.delete_field').on('click', function () {
            ...
            $('.modal-body #field_id').val(field_id);
            $('.modal-body #field').val(field);
        });
        ...
        $('#example-table tfoot tr th:nth-child(3)').html('<input class="form-control" type="text" placeholder="المجال" />');
